...
{
    fprintf(out,
        "Usage: " PGPID_MIP_NAME " scan [OPTIONS]... IMAGE...\n"
        "\n"
...
int pgpid_action_scan(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(a, "-p") || !strcmp(a, "--passphrase")) {
            ...
            pgpid_error("Try '" PGPID_MIP_NAME " scan --help' for more information.");
            return PGPID_USAGE;
        } else if (nimages < 64) {
            images[nimages++] = a;
        }
    ...
    if (given_workdir) {
        ...
        snprintf(workdir, sizeof workdir, "/tmp/pgpid-mip-scan.XXXXXX");
        if (!mkdtemp(workdir)) {
