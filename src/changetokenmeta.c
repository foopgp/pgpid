...
{
    fprintf(out,
        "Usage: " PGPID_MIP_NAME " change_token_meta [OPTIONS]... NEW_METADATA\n"
        "\n"
...
int pgpid_action_change_token_meta(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(a, "-A") || !strcmp(a, "--admincode")) {
            ...
            pgpid_error("Try '" PGPID_MIP_NAME " change_token_meta --help' for more information.");
            return PGPID_USAGE;
        } else if (!value) {
            value = a;
        }
    ...
    if (!strcmp(field, "name")) {
        ...
        char ring[] = "/tmp/pgpid-mip-url-XXXXXX";
        if (!mkdtemp(ring)) {
