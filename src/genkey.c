...
{
    fprintf(out,
        "Usage: " PGPID_MIP_NAME " gen_key [OPTIONS]... EMAIL\n"
        "\n"
...
int pgpid_action_gen_key(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(a, "-n") || !strcmp(a, "-N") || !strncmp(a, "--name", 6)) {
            ...
            pgpid_error("Try '" PGPID_MIP_NAME " gen_key --help' for more information.");
            return PGPID_USAGE;
        } else if (!email) {
            email = a;
        }
    ...
    if (!given_eid) {
        ...
        pgpid_error("  " PGPID_MIP_NAME " gen_u4 --surname … --given-names … --birth-date … --birth-country …");
        return PGPID_USAGE;
    }
