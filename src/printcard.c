...
{
    fprintf(out,
        "Usage: " PGPID_MIP_NAME " print_card [OPTIONS]... [NAME|EMAIL|KEYID|U4|U5]\n"
        "\n"
...
int pgpid_action_print_card(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(a, "-P") || !strcmp(a, "--print") || !strcmp(a, "--printer")) {
            ...
            pgpid_error("Try '" PGPID_MIP_NAME " print_card --help' for more information.");
            return PGPID_USAGE;
        } else if (!target) {
            target = a;
        }
    ...
    char workdir[512] = "/tmp/pgpid-mip-card.XXXXXX";
    if (!mkdtemp(workdir)) {
